...
class Facebook_BatchRequest
{
	...
	function __construct(array $queries = array(), $params = array())
	{
	...
	public static function queries(array $queries = array(), $params = array())
	{
	...
	function execute()
	{
		...
		foreach(array_chunk($this->queries,Config::get('facebook.batch_chunk_size') ?: 50) as $query_block){
			...
			$query = array_merge($this->params,array('batch' => json_encode($query_block)));
